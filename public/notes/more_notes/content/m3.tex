...
\end{equation}
Where $\mathcal{T}$ of index $k$ is a tensor representing a manifold of four dimensional properties. And $C$ is a residue constant in a arbitrary n-plane. Moreover the inverse function of $\mathcal{B}_{i}(n)$ is:
\begin{equation}
...
\begin{equation}
	\mathcal{Z}_t = \frac{1}{4} \nabla \mathcal{R}_{t}.
\end{equation}
And $\mathcal{D}_t$ be a tensor of the same order as $\mathcal{Z}$. Let following equation be in a linear 3-manifold space in $\mathbb{R}$:
\begin{equation}
	\Delta \mathcal{X} = \sum_{n=1}^{\infty} \sum_{t=1}^{\infty} {\mathcal{T}}\Bigl(\Bigl( \int_{n}^{p} Q(x) \, dx \Bigr) + \mathcal{D}_t\Bigr) + \mathcal{Z}_t.
\end{equation}
\end{definition}

\begin{remark}
	\label{rem:eq-tensor-p}
	Per Definition \ref{def:eq-tensor}, $p \coloneq + \infty$.
\end{remark}

\begin{remark}
	\label{rem:eq-tensor}
	Per Definition \ref{def:eq-tensor}, $\mathcal{D}_t$ is a tensor that satisfies the following: $\mathcal{D}_t \neq \langle \cdot \rangle$.
\end{remark}

\subsection{Several theorems per \ref{def:eq-tensor}:}
